...
const loadingTextOutlineWidthPx = 4;
...
export function LoadingContent({
	...
	return (
		<LoadingContentBase
			className={cn("grid place-items-center gap-[1.667rem]", className)}
			{...props}
		/>
	);
}
...
export function LoadingText({
	...
	return (
		<svg
			...
			role="img"
			className={cn("block h-[1.467rem] w-40 overflow-visible", className)}
			style={style}
		>
			<text
				x="50%"
...
export function LoadingSpinner({
	...
	return (
		<div
			className={cn("relative h-[3.333rem] w-[3.333rem]", className)}
			{...props}
		>
			<svg
				viewBox="0 0 26 25"
				...
				aria-hidden="true"
				className="absolute top-1/2 left-1/2 h-[1.133rem] w-[1.133rem] -translate-x-1/2 -translate-y-1/2"
			>
				<path
			...
			{dotPositions.map((position, index) => (
				<span
					...
					className="absolute top-1/2 left-1/2 h-[0.667rem] w-[0.667rem] rounded-full border-[1.33px] border-ll-loading-border bg-ll-white animate-[ll-loading-dot_1.2s_linear_infinite]"
					style={{
